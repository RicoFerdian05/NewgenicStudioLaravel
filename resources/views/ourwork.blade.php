<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Our Works</title>

    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
	<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.0/font/bootstrap-icons.css">
    <link rel="stylesheet" href="css/ourwork.css">

    <!-- Family Font Inter -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter&display=swap" rel="stylesheet">
</head>

<body>
    <div class="navbar">
        <div class="container">
                <div class="col-md-6" >
                    <img src="../icons/logo_newgenic.png" alt="">&nbsp;
                    <span class="title align-middle"><span class="sub-title">new</span>genic studio</span>
                </div>
                <div class="col-md-6">
                    <ul>
                        <li><a href="/">Home</a></li>
                        <li><a href="/web-service" >Service</a></li>
                        <li><a href="/team">Team</a></li>
                        <li><a href="/ourwork">Our Work</a></li>
                        <li><a href="/lets-talk"><button>Lets Talk</button></a></li>
                        <li>En</li>
                    </ul>
                </div>  
        </div>
    </div>

    <center>
    <div class="cont1">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <img src="../images/Designer Giving A Keynote 2.png">
                </div>
                <div class="col-md-6">
                    <div class="side">
                        <div class="name">
                            <p>PT. Agung Sedoyo Group</p>
                        </div>
                        <h4>Find The best Modern Apartment In The World</h4>
                        <a>UI/UX Strategy</a> 
                        <a>Website</a>
                        <a>Wordpress</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="cont2">
        <div class="container">
            <h2>
                Feature Project
            </h2>
            <div class="row">
                <div class="col-md-2">
                    <h5>All</h5>
                </div>
                <div class="col-md-2">
                    <h5>Website</h5>
                </div>
                <div class="col-md-2">
                    <h5>UI/UX Strategy</h5>
                </div>
                <div class="col-md-3">
                    <h5>Digital Marketing</h5>
                </div>
                <div class="col-md-3">
                    <h5>Social Media Analyst</h5>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4" style="margin-left:11px">
                    <div class="image">
                        <img src="../images/Coding.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="image">
                        <img src="../images/Group-2.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4" style="margin-right:0px;">
                    <div class="image">
                        <img src="../images/Group-3.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4" style="margin-left:11px">
                    <div class="image">
                        <img src="../images/Group-4.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="image">
                        <img src="../images/Group-5.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4" style="margin-right:0px;">
                    <div class="image">
                        <img src="../images/Group-6.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4" style="margin-left:11px">
                    <div class="image">
                        <img src="../images/Group-7.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="image">
                        <img src="../images/Group-8.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
                <div class="col-md-4" style="margin-right:0px;">
                    <div class="image">
                        <img src="../images/Group-9.png">
                    </div>
                    <div class="isi">
                        <h4>Prima - Real Estate <br> Landing Page</h4>
                        <p>Having a website that’s easy to understand and to use should be essential for every business owner who use their website</p>
                    </div>
                </div>
            </div>
            
            <div class="row">
                <div class="col">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                          <li class="page-item">
                            <a class="page-link ps-4 pe-4 border-0" href="#" aria-label="Previous">
                              <span aria-hidden="true">&laquo;</span>
                            </a>
                          </li>
                          <li class="page-item"><a class="page-link ps-4 pe-4 border-0" href="#">1</a></li>
                          <li class="page-item"><a class="page-link ps-4 pe-4 border-0" href="#">2</a></li>
                          <li class="page-item"><a class="page-link ps-4 pe-4 border-0" href="#">3</a></li>
                          <li class="page-item"><a class="page-link ps-4 pe-4 border-0" href="#">4</a></li>
                          <li class="page-item"><a class="page-link ps-4 pe-4 border-0" href="#">...</a></li>
                          <li class="page-item">
                            <a class="page-link ps-4 pe-4 border-0" href="#" aria-label="Next">
                              <span aria-hidden="true">&raquo;</span>
                            </a>
                          </li>
                        </ul>
                      </nav>
                </div>
            </div>
            
        </div>
    </div>
</center>

<div class="footer1">
    <div class="container">
        <div class="row">
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-4">
                        <img src="../images/Illustration 2.png">
                    </div>
                    <div class="col-md-8">
                        <h4>Join 569 more people in the waitlist</h4>
                        <button>Create a project</button>
                    </div>
                </div>
            </div>
            <div class="col-md-6" style="margin-left:160px;">
                <div class="row">
                    <div class="col-md-4">
                        <img src="../images/Illustration 4.png">
                    </div>
                    <div class="col-md-8">
                        <h4>You want work with us?</h4>
                        <button class="spc-button">Join our team</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="footer2">
    <div class="container">
        <div class="row">
            <div class="col-md-3" style="margin-left: 0px;">
                <h2><span class="title">N</span>ewgenic</h2>
                <h5>Newgenic Studio is committed to helping customers reach their business dreams. Providing perfect strategy, innovative creations and effective digital marketing solutions</h5>
                <img src="../icons/Social Icons.png" style="margin-left:0px">
                <img src="../icons/Social Icons-2.png">
                <img src="../icons/Social Icons-3.png">
                <img src="../icons/Social Icons-4.png">
            </div>
            <div class="col-md-3">
                <h6>Quick Links</h6>
                <p>Portofolio</p>
                <p>Blogs</p>
                <p>About</p>
                <p>Press</p>
                <p>Careers</p>
            </div>
            <div class="col-md-3">
                <h6>Services</h6>
                <p>UI/UX Design</p>
                <p>App Development</p>
                <p>Web Development</p>
                <p>Quality Assurance</p>
                <p>Machine Learning</p>
            </div>
            <div class="col-md-3">
                <h6>Reach Us</h6>
                <p><img src="../icons/logos_linkedin-icon.png" class="spc">hello@landify.co</p>
                <p><img src="../icons/logos_facebook.png" class="spc">Newgenic studio</p>
                <div class="row">
                    <div class="col-md-1">
                        <img src="../icons/address.png" class="spc">
                    </div>
                    <div class="col-md-3">
                        <p class="p-spc">772 Lyonwood Ave Walnut, CA 91789</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

</body>

